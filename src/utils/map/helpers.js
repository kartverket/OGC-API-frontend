...
export function getFeature(map, id) {
  ...
  return vectorSource.getFeatures().find((feature) => feature.getId() === id) || null;
}
...
function getMaxBbox() {
  const maxBbox = (MAX_BBOX ?? '').split(',').map((coord) => parseFloat(coord.trim()));

  return Array.isArray(maxBbox) && maxBbox.length === 4 ? maxBbox : null;
}
