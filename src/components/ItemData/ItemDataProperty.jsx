...
export default function ItemDataProperty({ data, level = 0, label = null, classNames = {} }) {
  ...
  if (data === null || typeof data !== 'object') {
    return (
      <div className={classNames.row ?? styles.row}>
        <div className={classNames.label ?? styles.label} style={{ marginLeft: indent }}>
          {label}
        </div>
  ...
  return (
    <div>
      {label && (
        <div className={styles.label} style={{ marginLeft: indent }}>
          {label}
        </div>
      )}

      {Object.entries(data).map(([key, value]) => (
        <ItemDataProperty key={key} data={value} level={level + 1} label={key} classNames={classNames} />
      ))}
    </div>
  );
}
