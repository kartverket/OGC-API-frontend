...
export default function DownloadPanel({ collectionId, downloadConfig }) {
  ...
  const activeFilter = formatScope !== 'full' ? areaFilters.find((f) => f.type === formatScope) : null;
  ...
  useEffect(() => {
    ...
    fetch(`/collections/${activeFilter.collection}/items?limit=500&f=json`)
      ...
      .then((data) => {
        if (cancelled) return;
        const names = [
          ...new Set((data.features ?? []).map((f) => f.properties?.[activeFilter.name_field]).filter(Boolean)),
        ].sort();
        setAreaNames(names);
  ...
  async function handleDownload() {
    ...
    try {
      const processId =
        formatBase === 'gpkg' && formatScope === 'full'
      ...
      const inputs = activeFilter
        ? {
            ...
            area_type: formatScope,
            area_name: areaName,
            area_name_field: activeFilter.name_field,
          }
        : { collection_id: collectionId };
    ...
    <Card className={styles.panel}>
      <CardBlock className={styles.cardBlock}>
        ...
        <div className={styles.controls}>
          <Field>
            <Label htmlFor="dl-format" data-size="sm">
              Format
            </Label>
            <Select id="dl-format" value={format} onChange={handleFormatChange} data-size="sm" disabled={busy}>
          ...
          {activeFilter && (
            <Field>
              <Label htmlFor="dl-area" data-size="sm">
                {activeFilter.label}
              </Label>
              <Select
                id="dl-area"
        ...
        {status && <div className={`${styles.status} ${styles[`status--${status.type}`]}`}>{status.message}</div>}
      </CardBlock>
    </Card>
  );
}
