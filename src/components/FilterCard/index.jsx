...
export default function FilterCard({ data }) {
  ...
  const controlType = useMemo(
    () => getControlTypeFromField(selectedField, data.queryables),
    [selectedField, data.queryables],
  );
    ...
    <Card className={styles.filterCard}>
      ...
      {selectedFilters.length > 0 && (
        <div className={styles.selectedFilters}>
          <Chip.Button onClick={removeAllFilters} data-size="sm">
            Fjern alle filtre
          </Chip.Button>
          {selectedFilters.map(({ field, value }, index) => (
            <Chip.Removable key={field} onClick={() => removeFilter(index)} data-size="sm">
              {field}={value}
            </Chip.Removable>
          ))}
        </div>
      ...
      <div className={styles.filters}>
        <div className={styles.fields}>
          <Field className={styles.fieldSelect}>
            ...
            <Select id="field-select" value={selectedField} onChange={handleFieldSelectChange} data-size="sm">
              <Select.Option value="" disabled>
                Velg fra listen...
              </Select.Option>
              {fields.map((field) => (
          ...
          <Button onClick={addFilter} data-size="sm" disabled={selectedField === '' || filterValue === ''}>
            Legg til filter
          </Button>
        </div>
        ...
        <div className={styles.bbox}>
          ...
          <div className={styles.inputs} style={{ display: bboxEdit ? 'flex' : 'none' }}>
            <div>
              <Label htmlFor="minLon" data-size="sm">
                Min. lon
              </Label>
              <Input
                type="number"
            ...
            <div>
              <Label htmlFor="minLat" data-size="sm">
                Min. lat
              </Label>
              <Input
                type="number"
            ...
            <div>
              <Label htmlFor="maxLon" data-size="sm">
                Maks. lon
              </Label>
              <Input
                type="number"
            ...
            <div>
              <Label htmlFor="maxLat" data-size="sm">
                Maks. lat
              </Label>
              <Input
                type="number"
          ...
          <div className={styles.buttons}>
            {!bboxEdit && (
              <Button onClick={startEditBbox} variant="secondary" data-size="sm">
            ...
            {bboxEdit && (
              <>
                <Button onClick={cancelEditBbox} variant="secondary" data-size="sm">
                  <XMarkIcon title="Avbryt" fontSize="24px" />
                  Avbryt redigering
                </Button>

                <Button onClick={addBboxFilter} data-size="sm">
