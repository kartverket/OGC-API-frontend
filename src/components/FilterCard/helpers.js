...
export function getFields(selectedFilters, queryables) {
  return Object.keys(queryables.properties).filter(
    (key) => key !== 'geometry' && !selectedFilters.some((filter) => filter.field === key),
  );
}
...
export function getControlTypeFromField(selectedField, queryables) {
  ...
  const [, property] = Object.entries(queryables.properties).find((entry) => entry[0] === selectedField);
...
export function isWithinBounds(coordinate, index, viewBounds) {
  ...
  return index === 0 || index === 1 ? parsed >= viewBounds[index] : parsed <= viewBounds[index];
}
